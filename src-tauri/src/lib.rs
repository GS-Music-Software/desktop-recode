...
pub fn run() {
    tauri::Builder::default()
        ...
        .manage(rpc::init())
        .manage(tray::init())
        .invoke_handler(tauri::generate_handler![scan_dir, get_cover, stream_file, fetch_lyrics, fetch_ttml, search_tracks, search_albums, search_artists, get_album_tracks, get_artist_albums, download_track, yt_playlist_tracks, rpc_on, rpc_off, rpc_set, rpc_clr, rpc_cover, tray_set, check_ytdlp, install_ytdlp, check_ytdlp_update, check_ffmpeg, install_ffmpeg, sp_save_client_id, sp_load_client_id, sp_authorize, sp_load_tokens, sp_disconnect, sp_fresh_token, sp_playlists, sp_playlist_tracks, sp_liked_tracks, pl_list, pl_create, pl_rename, pl_delete, pl_get, pl_add_track, pl_remove_track, pl_cover, pl_set_cover, pl_favs, pl_fav_toggle, pl_reorder, pl_reorder_tracks, delete_track, get_audio_device, export_account, import_account, import_fill_playlists, lookup_cover])
        .setup(|app| {
            #[cfg(target_os = "linux")]
            {
