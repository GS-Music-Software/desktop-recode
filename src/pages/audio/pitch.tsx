...
function fmt_rate(v: number): string {
...
export function Pitch() {
  ...
  function on_track_click(e: React.MouseEvent) {
    ...
    update(pct_to_val(Math.max(0, Math.min(1, p))));
  }
  ...
  function on_thumb_down(e: React.MouseEvent) {
    ...
    function on_move(ev: MouseEvent) {
      ...
      const p = Math.max(0, Math.min(1, start_pct + dx / track_w));
      update(pct_to_val(p));
    }
  ...
  return (
    <div style={{
      ...
      >
        ...
        {SNAPS.map(s => {
          const sp = val_to_pct(s) * 100;
          return (
            <div key={s} style={{
              position: "absolute", left: `${sp}%`, top: "50%",
              transform: "translate(-50%, -50%)",
              width: s === DEFAULT ? 6 : 3,
              height: s === DEFAULT ? 6 : 3,
              borderRadius: "50%",
              background: s === DEFAULT ? c.w25 : c.w10,
            }} />
          );
        })}
      ...
      <div style={{ position: "relative", height: 16, marginTop: 8 }}>
        <span style={{ position: "absolute", left: 0, fontSize: 10, color: c.w20 }}>0.25x</span>
        <span style={{ position: "absolute", left: "50%", transform: "translateX(-50%)", fontSize: 10, color: c.w20 }}>1x</span>
        <span style={{ position: "absolute", right: 0, fontSize: 10, color: c.w20 }}>2x</span>
      </div>

      <p style={{ fontSize: 11, color: c.w20, textAlign: "center", marginTop: 12 }}>
        Drag to adjust · Double-click reset to restore
      </p>
    </div>
  );
}
