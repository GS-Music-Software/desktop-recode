...
export function YtdlpSetup({ on_ready }: { on_ready: () => void }) {
  ...
  if (status === "checking_ytdlp" || status === "checking_ffmpeg" || status === "ready") return null;
  ...
  if (status === "ytdlp_missing") {
    ...
    body = "yt-dlp is required for downloading and discovering music. It will be installed to your app data folder.";
    btn_label = "Install";
    ...
    body = "ffmpeg is required for audio processing and metadata embedding. It will be installed to your app data folder.";
    btn_label = "Install";
    btn_action = handle_install_ffmpeg;
  } else if (status === "installing_ffmpeg") {
    title = "Installing ffmpeg";
    body = "Downloading ffmpeg. This may take a minute as the download is larger.";
    loading = true;
  ...
  return (
    <div className="ytdlp-backdrop" style={{
      ...
      <div className="ytdlp-panel" style={{
        ...
        <p style={{
          fontSize: 14, color: c.w50, lineHeight: 1.6,
          ...(status === "error" ? { color: c.accent_70, wordBreak: "break-all" as const } : {}),
        }}>{body}</p>
